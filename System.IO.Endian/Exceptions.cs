    ...
    internal static class Exceptions
    {
            ...
            where T : struct, INumber<T>
        {
            if (!T.IsPositive(paramValue))
                throw new ArgumentOutOfRangeException(paramName, paramValue, Utils.CurrentCulture($"'{paramName}' must be greater than zero."));
        }
            ...
            where T : struct, INumber<T>
        {
            if (!(T.IsPositive(paramValue) || T.IsZero(paramValue)))
                throw new ArgumentOutOfRangeException(paramName, paramValue, Utils.CurrentCulture($"'{paramName}' must be greater than or equal to zero."));
        }
        ...
        /// <summary>
        /// Throws an exception if <paramref name="prop"/> does not have both a public <see langword="get"/> and a public <see langword="set"/> accessor.
        /// </summary>
        /// <param name="prop"></param>
        /// <exception cref="MissingMethodException" />
        [DebuggerHidden]
        public static void ThrowIfNotPublicGetSet(PropertyInfo prop)
        {
            if (prop.GetGetMethod() == null || prop.GetSetMethod() == null)
                throw new MissingMethodException(Utils.CurrentCulture($"The '{prop.Name}' property was marked for read/write but has no public get and/or set methods."));
        }
